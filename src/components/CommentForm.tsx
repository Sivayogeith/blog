...
import { Comment, replyToComment } from "../api/commentsAPI";
import { Dispatch, Ref, SetStateAction, useImperativeHandle, useState } from "react";

export interface CommentFormElement extends HTMLFormElement {
...
export default function CommentForm({
  ...
  const [show, setShow] = useState(!reply)

  const onSubmit = async ({ text }: CommentFormData) => {
    let result;
    if (reply) {
      result = await replyToComment(comment!.id, text);
    } else {
      result = await addComment(post!.slug, text);
    }
    if (result.status == 200) {
      if (reply) setShow(false)
      router.refresh();
      setValue("text", "");
  ...
  useImperativeHandle(ref, () => {
    return { setShow } as CommentFormElement;
  })
    ...
    <form onSubmit={handleSubmit(onSubmit)} className={`mt-4 ${className}`}>
      <div className="flex w-full">
        <textarea
          ...
          className="lg:w-[55vw] w-full rounded-e-none!"
        />
        <button
          className="border-y border-e border-s-0 rounded-s-none"
          disabled={!isValid}
        >
